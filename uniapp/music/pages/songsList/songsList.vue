	...
	<view class="songs-list">
		...
		<view class="list-hd">
			<view class="hd-info">
				<view class="pic boxShadow" >
					<image :src="info.coverImgUrl" mode=""></image>
				</view>
				<view class="desc">
					<view class="title">
						{{info.name}}
					...
					<view class="user">
						<view class="avatar">
							<image :src="info.creator&&info.creator.avatarUrl" mode=""></image>
						</view>
						<view class="nickname">
		...
		<view class="list-bd">
					...
					<view class="bd-content">
						<view class="item" @click="showPlayer(song.id)" v-for="(song,index) in list" :key="song.al.id">
							...
							<view class="song-desc">
								...
								<view class="author">
									<text v-for="(item,index) in song.ar" :key="item.id">{{item.name}}
...
.songs-list{
	...
	.list-bd{
		...
		.bd-title{
			...
			align-items: center;
			padding: 30rpx 0;
			// -webkit-sticky 兼容属性，-webkit-谷歌的兼容属性
